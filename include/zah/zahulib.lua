...
function floatrand(min,max) first=math.random(min,max-1) second=math.random(1,99) return tostring(first).."."..tostring(second) end

function tagrandpulse(max,start,tag,value1,value2,value3,value4,frequency) 
    tagstring = ""
    i = frequency
    for j=0,math.floor(max/i) do 
        if j%2==0 then tagstring = tagstring.."\\t("..(j*i)+start..","..((j+1)*i)+start..",\\"..tag..floatrand(value1,value2)..")" 
        else tagstring = tagstring.."\\t("..(j*i)+start..","..((j+1)*i)+start..",\\"..tag..floatrand(value3,value4)..")" end 
    end 
    return tagstring 
end
